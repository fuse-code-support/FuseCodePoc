# boot-code - Code interactively using CodeMirror
...
(defn theme-assets [] (sort (filter theme? raw-assets)))
(defn mode-assets [] (sort (filter mode? raw-assets)))
(defn addon-assets [] (sort (filter addon? raw-assets)))
(defn keymap-assets [] (sort (filter keymap? raw-assets)))
(defn codemirror-assets [] (sort (filter codemirror? raw-assets)))
...
(addon-assets)
(def some-addon-assets (take 8 (addon-assets)))
some-addon-assets

(defn compute2 [[asset-map last-asset] [asset-path sri]]
  (let [next-asset (asset-name asset-path)
        dom-fn (asset->dom-fn asset-path sri)]
    [[asset-map last-asset asset-path sri] [next-asset dom-fn]]))

(first some-addon-assets)
(second some-addon-assets)

(->> (compute-asset-map [{} ""] (first some-addon-assets))
     (compute-asset-map (second some-addon-assets)))

(def themes (first (reduce compute-asset-map [{} ""] (theme-assets))))
(def modes (first (reduce compute-asset-map [{} ""] (mode-assets))))
(def addons (first (reduce compute-asset-map [{} ""] (addon-assets))))
(def keymaps (first (reduce compute-asset-map [{} ""] (keymap-assets))))

(def codemirror (first (reduce compute-asset-map [{} ""] (codemirror-assets))))
